    ...
    try: 
		...
		def subtourelim(model, where):
			if where == GRB.callback.MIPSOL:
				...
				print "best obj", model.cbGet(GRB.callback.MIPSOL_OBJBST)
				print "node count", model.cbGet(GRB.callback.MIPSOL_NODCNT)
				vals = model.cbGetSolution(model.getVars())
				varsx = model.getVars()[n*n:]
				varsx1 = [varsx[i:i+n] for i in range(0,len(varsx),n)]
				t1 = [vals[i:i+n] for i in range(0,n*n,n)]#vals[:n*n]
						...
						if t1[area][j]>=1-tol:#==1:##
							...
							while True:
								...
								if x1[j][k]>=1-tol:#==1:
									...
									break
								k += 1
							#VERIFICACIÓN QUE NO DEBERÍA HACERSE
							if num.count(j)!=0:##
								b =num.remove(j)
							#print "hey"
