...
def execArisel(y, w, unsigned int pRegions, unsigned int inits=3, list initialSolution=[], unsigned int convTabu=0, unsigned int  tabuLength=10):
    ...
    Openshaw's AZP-tabu [Openshaw_Rao1995]_. In ARISeL the construction of a initial feasible
    solution is repeated several times (inits) before running Tabu
    Search algorithm [Glover1977]_.
    ...
    :type std: binary
    :keyword inits: number of initial feasible solutions to be constructed before applying Tabu Search.
    ...
    # #  EVALUATE SOLUTION 
    # rmObjInfo = rm.objInfo
    
    # if rmObjInfo < extendedMemoryObjInfo:
    # extendedMemory.updateExtendedMemory(rm)
    # extendedMemoryObjInfo = rmObjInfo
